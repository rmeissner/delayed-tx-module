...
contract DelayedTxModule {
    ...
    function updateConfig(address announcer, uint64 delaySec, uint16 validityDurationMin, bool requireAnnouncer, bool notifyExecutor)
        public
    {
        // Note: msg.sender is the executor
        configs[msg.sender][announcer] = Config(delaySec, validityDurationMin, requireAnnouncer, notifyExecutor);
    ...
    function announceTransaction(address executor, address to, uint256 value, bytes memory data, Enum.Operation operation, uint256 nonce) 
        public 
    {  
        announceTransactionWithDuration(executor, to, value, data, operation, nonce, 0);
    }
    
    function announceTransactionWithDuration(address executor, address to, uint256 value, bytes memory data, Enum.Operation operation, uint256 nonce, uint16 validityDurationMin) 
        public 
    {
        // Note: msg.sender is the announcer
        Config memory config = configs[executor][msg.sender];
        require(config.delaySec > 0, "Could not find valid config for executor and announcer");
        ...
        if (config.notifyExecutor) {
            bytes memory approveAnnouncement = abi.encodeWithSignature(
                "approveTransactionAnnouncement(address,uint256,bytes,uint8,uint256,address,uint64,uint16,bool)", 
                to, 
                value,
                data,
                operation,
    ...
    {
        // Note: msg.sender is the executor
        addAnnouncement(msg.sender, to, value, data, operation, nonce, announcer, execTime, validityDurationMin, requireAnnouncer);
    }
    ...
    function addAnnouncement(
        ...
        announcements[txHash] = Announcement(announcer, execTime, validityDurationMin, requireAnnouncer, false);
        ...
        public 
    {
        ...
        require(announcement.validityDurationMin == 0 || announcement.execTime + (announcement.validityDurationMin * 60) >= block.timestamp, "Announcement expired");
        // If the announcer is set we should check if the announcer is still enabled
        require(
            !announcement.requireAnnouncer || configs[executor][announcement.announcer].delaySec > 0, 
